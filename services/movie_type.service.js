...
import * as repo from "../repositories/movie_type.repo.js";
...
const invalidateCache = () => {
  Producer.deleteCache(`${CACHE_PREFIX.MOVIE_TYPES}:*`);
};
...
export const findAndPaginate = async (query) => {
  const cacheKey = buildCacheKey(CACHE_PREFIX.MOVIE_TYPES, query);
  ...
  if (!result.error) {
    await setCacheWithTTL(cacheKey, result, TTL.COLD);
  }
